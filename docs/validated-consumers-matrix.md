...
> **SUPERSEDED (2026-09-21):** la nota de estado de 2026-08 de abajo, que declara `anclora-talent` "pausado, fuera del alcance activo", queda contradicha por evidencia de código real (commits activos hasta 2026-09-21, ver [`ecosystem-audit/00-frontend-inventory-and-drift-audit.md`](./ecosystem-audit/00-frontend-inventory-and-drift-audit.md) §A2.5.5). Además, esta nota describe el consumo de `anclora-talent` como "sin fork estructural" — verificado como impreciso: `.ac-button` tiene overrides reales de anatomía y `.dashboard-button` es una familia paralela nunca derivada de él (§A2.5.4 del mismo documento; resuelto parcialmente en 0.5.0 con `.ac-button--compact`, ver [`CHANGELOG.md`](../CHANGELOG.md)). Las piezas listadas abajo como `canonical`/`validated` siguen siéndolo — el consumo real que las valida no se invalida por esto. Para el estado de adopción actual y verificado de todo el ecosistema (no solo los 2 consumidores aquí documentados), usar **[`ANCLORA-DESIGN-SYSTEM-PRODUCT-MATRIX.md`](../ANCLORA-DESIGN-SYSTEM-PRODUCT-MATRIX.md)**. Este documento se conserva sin editar como registro histórico; no se actualiza más.
