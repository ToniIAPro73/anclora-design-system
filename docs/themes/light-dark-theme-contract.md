...
- **`--accent-mint`**: referenciado por `editorStudio.css`/`editorShell.css`/`textEditor.css` sin definición en ningún lugar del paquete — descubierto durante esta auditoría (no introducido por ella). No se inventa un valor sin evidencia de qué "mint" debía representar — requiere una decisión de producto, registrada como seguimiento explícito (ver `docs/ecosystem-audit/00-frontend-inventory-and-drift-audit.md` cross-repo follow-ups y el CHANGELOG de esta versión).
