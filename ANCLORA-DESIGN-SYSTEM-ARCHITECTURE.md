...
- **Tokens semánticos conscientes de tema — CORRECCIÓN (2026-09-21, tras auditoría dedicada, ver `docs/ecosystem-audit/06-icon-only-and-theme-reconciliation.md`):** esta sección original de Fase 4 afirmaba que "ya existe la estructura correcta en `src/tokens/semantic.css` (bloques por tema)" — **esto era incorrecto y se corrige aquí**. Verificado por inspección directa: `src/tokens/semantic.css`, `src/tokens/core.css`, `src/foundations/*.css` y `src/themes/product.css` no contienen **ningún** bloque `[data-theme='light']`, `.light`, `@media (prefers-color-scheme: light)` ni `color-scheme: light` — el paquete es, en su estado ejecutable real, **solo modo oscuro**. La única evidencia de "modo claro" en el código son dos primitivos de color con sufijo `-light` (`--portfolio-gold-light`, `--portfolio-glass-light`, variantes puntuales de la familia Portfolio, no un sistema de tema) y un comentario que confirma explícitamente que las páginas de demostración del propio paquete "fuerzan `data-theme='dark'` por defecto" (`src/themes/product.css:9`). **Clasificación: gap de arquitectura genuino, no un hueco de catálogo** — el contrato de tema descrito arriba (atributo `data-theme`, preferencia de sistema, persistencia, hidratación SSR) es la **arquitectura objetivo**, todavía no implementada a nivel de tokens ejecutables. Los dos consumidores reales (`anclora-talent`, `anclora-command-center`) resuelven esto hoy definiendo su **propia** paleta clara completa, de forma totalmente independiente entre sí y sin ningún contrato compartido del DS — ni siquiera comparten qué tokens semánticos deberían tener contraparte clara. Seguimiento propuesto (no ejecutado en esta misión, alcance propio): **"Wave 1.6 — Light Theme Foundation"**, con un objetivo acotado y verificable — definir el bloque `[data-theme='light']` de `src/tokens/semantic.css` para los tokens semánticos ya existentes (no primitivos nuevos por producto), y extender `preview/components-canonical.html` para demostrar ambos temas realmente. No se ejecuta aquí porque es trabajo de diseño real (decidir valores de color, no solo mecanismo), no una reconciliación de contrato.
