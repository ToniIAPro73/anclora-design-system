...
- **Tokens semánticos conscientes de tema — IMPLEMENTADO (0.7.0, Wave 1.6).** Esta sección pasó por dos estados documentados: originalmente (Fase 4) afirmaba, incorrectamente, que la estructura ya existía; el 21/09 una auditoría dedicada corrigió eso a "gap genuino, solo modo oscuro" (evidencia entonces: cero bloques `[data-theme='light']` en `src/tokens/`, `src/foundations/`, `src/themes/`). Wave 1.6 implementó el gap: `src/tokens/semantic.css` ahora tiene `:root, [data-theme="dark"] { ... }` (idéntico al bloque pre-0.7.0, sin cambios de valor — compatibilidad retroactiva total) y un `[data-theme="light"] { ... }` real, con cada par texto/superficie crítico verificado contra WCAG 2.2 AA por `scripts/verify-theme-contrast.mjs` (matemática de contraste real, no estimación visual). Contrato completo, con evidencia de los dos consumidores reales y ejemplos: `docs/themes/light-dark-theme-contract.md`. **Lo que sigue sin implementar, explícitamente fuera de esta versión:** sombras por tema (`--shadow-*` permanece invariante), y el token `--accent-mint` (usado por 3 componentes de la familia editorial, nunca definido — descubierto, no resuelto, por esta auditoría; requiere una decisión de producto antes de asignarle un valor).
